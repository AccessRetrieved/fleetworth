...
During the recording, the app collects **3–7 JPEG photos**, waiting about ten seconds of clear framing between automatic captures so the user can move. Those photos are the only visual inputs for future extraction and pricing. The continuous session video is retained separately as a live-capture/authenticity record, shown for review after recording, and is not analysed for the price.
